...
def write_query_rel(args, pid2offset, query_file, positive_id_file, out_query_file, out_id_file):
    ...
    with gzip.open(query_positive_id_path, 'rt', encoding='utf8') if positive_id_file[-2:] == "gz" else open(query_positive_id_path, 'r', encoding='utf8') as f:
        if args.data_type == 0 or args.dataset != "marco":
...
def preprocess(args):
    ...
    if args.dataset == "marco":
        ...
        with open(os.path.join(args.data_dir, "..", "marco_preprocessed", "pid2offset.pickle"), "rb") as f:
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="We use rdot_nll in this work."
    )
    parser.add_argument(
        "--model_name_or_path",
        default=None,
        type=str,
        required=True,
        help="We use roberta-base in this work."
    )
    parser.add_argument(
        "--max_seq_length",
        default=128,
    ...
    parser.add_argument(
        "--dataset",
        choices=["marco", "trec19psg", "trec20psg", "dlhard"],
