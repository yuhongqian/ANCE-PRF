...
def EvalDevQuery(
    ...
    evaluator = pytrec_eval.RelevanceEvaluator(
        convert_to_string_id(dev_query_positive_id), {'map', 'map_cut', 'ndcg_cut'})
    ...
    for k in result.keys():
        ...
        for metric in metric_results.keys():
            metric_results[metric] += result[k][metric]
            # ndcg += result[k]["ndcg_cut_10"]
    for metric in metric_results.keys():
        metric_results[metric] /= eval_query_cnt
    # final_ndcg = ndcg / eval_query_cnt
    final_ndcg = metric_results["ndcg_cut_3"]

    # print("Rank:" + str(args.rank) + " --- ANN NDCG@10:" + str(final_ndcg))
    print("Rank:" + str(args.rank) + " --- ANN Results:" + str(metric_results))
...
def ann_data_gen(args):
    ...
    while args.end_output_num == -1 or output_num <= args.end_output_num:
        ...
        if next_checkpoint == last_checkpoint:
            ...
            generate_new_ann(args, next_checkpoint, latest_step_num)
            if args.inference:
                break
